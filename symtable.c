...
// Lab 7
// 10/30/22
// Adding symbol table to lab 6
//
// changes: added GTEMP, TEMP_CREATE, cases for each type for PrintSym
...
int GTEMP = 0; // counter for creating temps

// function for creating names for temperary symbols using GTEMP as a counter
char *TEMP_CREATE() {
	char s[100];
	sprintf(s, "_L%d", GTEMP);
	GTEMP++;
	return(strdup(s));
}
	...
	char *string,*display_type; 
	 switch(s->SubType) {
		 ...
		 case ID_Sub_Type_Method: string = "Method " ; break;
	         case ID_Sub_Type_Array: string = "Array " ; break;			  //MISSING
